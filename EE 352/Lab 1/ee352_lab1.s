...
maxL:	add $s0,$s0,$t2		# move address accordingly to the counter
		...
		bgt  $t1, 20, input     # Check boundary on user input -- if invalid, restart
     	blt  $v0, $zero, input   # Check boundary on user input -- if invalid, restart
#### Print Inputterd List ####
		...
		move $t4, $t1
		mulo $t4,$t4,2
PIL:	add $s0,$s0,$t2		# move address accordingly to the counter
		add $t2,$t2, 1		# increment counter
		lb $a0,($s0)	
		li $v0, 11
		syscall
		la $s0,buf1			# restores buf1 starting address
		blt $t2,$t4,PIL		# Branch 
